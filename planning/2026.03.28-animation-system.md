...
> **2026-04-18**: Updated to reflect the implemented system. Added sections on two-time-base architecture, quantized scrubbing, adaptive compression, scrub-freeze detection, and the fix for mid-animation freeze capture (bug where wheels froze at half-way positions when transitioning from single-step to hold-scrub).
...
interface HandState {
    ...
    preScrubAngle: number | null;        // Captured angle at scrub start
    firstScrubTarget: number | undefined; // Target from first scrub tick
    scrubFrozen: boolean;                // True if frozen during scrub
}
```
...
- No compression or scrub-freeze logic
...
**Purpose**: When scrubbing at high speed (e.g., stepping by hour), some hands don't change their value (e.g., the second hand always evaluates to `0` because seconds are always 0 at hour boundaries). Without scrub-freeze, these hands would snap to angle 0, losing their pre-scrub visual position.

**Algorithm** (quantized mode only):
1. `resetHandSchedules` captures each hand's current rendered angle as `preScrubAngle`
2. On the **first** quantized tick: record `firstScrubTarget = newTarget`, force hand back to `preScrubAngle`, and wait
3. On the **second** tick: compare `newTarget` with `firstScrubTarget`:
   - **Same**: Hand is "frozen" — its expression always returns the same value during this scrub. Keep displaying `preScrubAngle` and stop re-evaluating.
   - **Different**: Hand is active — clear freeze state and fall through to normal animation.

**Critical detail**: When capturing `preScrubAngle`, if the hand is mid-animation, we must capture the animation **target** (`targetValue`) not the current interpolated position. Otherwise, frozen hands can get stuck at half-way positions when transitioning from a single-step tap to a hold-scrub.
...
| `resetHandSchedules(states)` | Unfreeze schedules + capture preScrubAngle |
| `clearScrubFreeze(states)` | Clear freeze state when leaving scrub mode |
...
- During scrubbing: frozen hands (e.g., seconds when stepping by hour) should hold their pre-scrub angle, not snap to 0
- Single-step followed by hold-scrub should not leave wheels at half-way positions
